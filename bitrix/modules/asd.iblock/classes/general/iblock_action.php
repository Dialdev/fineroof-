...
class CASDiblockAction {
	...
	public static function OnBeforePrologHandler() {
		...
		if ($action=='asd_prop_export' && $BID>0 && check_bitrix_sessid() &&
		...
		if ($action=='asd_reverse' && $IBLOCK_ID>0 && check_bitrix_sessid() &&
		...
		if ($bRightPage && check_bitrix_sessid() && !empty($_REQUEST['ID']) &&
			...
			($gridAction=='asd_copy' || $gridAction=='asd_move') &&
			...
			if ($bSectPage || $bMixPage) {
				...
				foreach (array_keys($sections) as $index) {
					if (!CASDIblockRights::IsSectionSectionCreate($intDestIBlockID, $sections[$index])) {
						unset($sections[$index]);
					}
				}
				$boolCreateSection = !empty($sections);
			}
			if (!empty($sections)) {
