...
if(strlen($arParams["SUBMIT_NAME"]) <= 0)
	$arParams["SUBMIT_NAME"] = 'iblock_submit';
...
{
	// process POST data
	if (check_bitrix_sessid() && (!empty($_REQUEST[$arParams['SUBMIT_NAME']]) || !empty($_REQUEST["iblock_apply"])))
	{
		...
		if (empty($arResult["ERRORS"]))
		{
			if (!empty($_REQUEST[$arParams['SUBMIT_NAME']]))
			{
				...
				else
				{
					...
					{
						$sRedirectUrl = $APPLICATION->GetCurPageParam("", array("edit", "CODE", "strIMessage","button"), $get_index_page=false);
					}
			...
			else
			{
				if (strlen($SEF_URL) > 0)
					...
					$sRedirectUrl = $APPLICATION->GetCurPageParam("edit=Y&CODE=".$arParams["ID"], array("edit", "CODE", "strIMessage", "button"), $get_index_page=false);
			}
			...
			$sRedirectUrl .= '&edit_button='.$arParams['SUBMIT_NAME'];
